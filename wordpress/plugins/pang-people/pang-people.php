...
function pang_people_register_types() {
	register_post_type( 'pang_person', array(
		'labels' => array(
			'name' => 'People', 'singular_name' => 'Person',
			'add_new_item' => 'Add Person', 'edit_item' => 'Edit Person',
		),
		'public' => true, 'show_in_rest' => true, 'menu_icon' => 'dashicons-groups',
		'supports' => array( 'title', 'thumbnail', 'page-attributes' ),
		'rewrite' => array( 'slug' => 'people' ), 'has_archive' => false,
	) );

	register_taxonomy( 'pang_person_category', array( 'pang_person' ), array(
		'labels' => array( 'name' => 'People Categories', 'singular_name' => 'People Category' ),
		'public' => true, 'show_in_rest' => true, 'hierarchical' => false, 'rewrite' => false,
	) );
}
...
function pang_people_activate() {
	...
	foreach ( array( 'Faculty','Researchers','PhD Students','Collaborators','Past Members' ) as $term ) {
		if ( ! term_exists( $term, 'pang_person_category' ) ) wp_insert_term( $term, 'pang_person_category' );
	}
	flush_rewrite_rules();
}
...
function pang_people_add_meta_boxes() {
	add_meta_box( 'pang_person_details', 'PANG Profile Details', 'pang_people_render_meta_box', 'pang_person', 'normal', 'high' );
}
...
function pang_people_save_meta( $post_id ) {
	...
	foreach ( array('academic_position','affiliation','biography','research_interests','orcid','google_scholar','scopus') as $key ) {
		...
		if ( in_array($key,array('orcid','google_scholar','scopus'),true) ) $value = esc_url_raw($raw);
		elseif ( in_array($key,array('biography','research_interests'),true) ) $value = sanitize_textarea_field($raw);
...
function pang_people_grid_shortcode( $atts ) {
	...
	$args = array(
		'post_type'=>'pang_person','posts_per_page'=>-1,
		'orderby'=>array('menu_order'=>'ASC','title'=>'ASC'),
	);
	if ( $atts['category'] ) {
		$args['tax_query']=array(array(
			'taxonomy'=>'pang_person_category','field'=>'slug','terms'=>sanitize_title($atts['category'])
		));
	}
	$q=new WP_Query($args);
	if(!$q->have_posts()) return '';
	ob_start(); echo '<div class="pang-people-grid">';
	while($q->have_posts()){ $q->the_post(); $id=get_the_ID();
		...
		if(has_post_thumbnail()) echo get_the_post_thumbnail($id,'medium_large',array('class'=>'pang-person-card__image'));
		echo '<div class="pang-person-card__body">';
		if($category) echo '<span class="'.esc_attr(pang_people_badge_class($category)).'">'.esc_html($category).'</span>';
		echo '<h3>'.esc_html(get_the_title()).'</h3>';
		if($position) echo '<p class="pang-person-card__position">'.esc_html($position).'</p>';
		if($affiliation) echo '<p class="pang-person-card__affiliation">'.esc_html($affiliation).'</p>';
		echo '</div></a></article>';
	}
...
function pang_people_profile_content( $content ) {
	...
	<div class="pang-profile-head">
		<div class="pang-profile-photo"><?php if(has_post_thumbnail()) the_post_thumbnail('medium_large'); ?></div>
		<div class="pang-profile-summary">
			<?php if($category): ?><span class="<?php echo esc_attr(pang_people_badge_class($category)); ?>"><?php echo esc_html($category); ?></span><?php endif; ?>
			<?php if($position): ?><p class="pang-profile-position"><?php echo esc_html($position); ?></p><?php endif; ?>
			<?php if($affiliation): ?><p class="pang-profile-affiliation"><?php echo esc_html($affiliation); ?></p><?php endif; ?>
		</div>
	</div>
	<?php if($biography): ?><section class="pang-profile-section"><h2>Biography</h2><p><?php echo nl2br(esc_html($biography)); ?></p></section><?php endif; ?>
	<?php if($interests): ?><section class="pang-profile-section"><h2>Research Interests</h2><ul>
	<?php foreach(preg_split('/\R+/',$interests) as $interest){$interest=trim($interest);if($interest)echo '<li>'.esc_html($interest).'</li>';} ?>
	</ul></section><?php endif; ?>
	<?php if($orcid||$scholar||$scopus): ?><nav class="pang-profile-links" aria-label="External academic profiles">
	<?php if($orcid): ?><a href="<?php echo esc_url($orcid); ?>" target="_blank" rel="noopener">ORCID</a><?php endif; ?>
	<?php if($scholar): ?><a href="<?php echo esc_url($scholar); ?>" target="_blank" rel="noopener">Google Scholar</a><?php endif; ?>
	<?php if($scopus): ?><a href="<?php echo esc_url($scopus); ?>" target="_blank" rel="noopener">Scopus</a><?php endif; ?>
	</nav><?php endif;
	return ob_get_clean();
}
...
function pang_people_order_menu() {
	add_submenu_page('edit.php?post_type=pang_person','Order People','Order People','edit_pages','pang-people-order','pang_people_order_page');
}
...
function pang_people_order_assets($hook) {
	...
	wp_add_inline_style('wp-admin', '.pang-sortable{max-width:760px;margin:12px 0 30px}.pang-sort-item{background:#fff;border:1px solid #ccd0d4;padding:12px 16px;margin:6px 0;cursor:move;border-radius:5px}.pang-sort-item:before{content:"☰";margin-right:12px;color:#777}.pang-sort-placeholder{height:44px;border:2px dashed #8c8f94;margin:6px 0}.pang-order-cat{margin-top:28px}');
}
add_action('admin_enqueue_scripts','pang_people_order_assets');

function pang_people_order_page() {
	if(!current_user_can('edit_pages')) return;
	$categories=array('Faculty','Researchers','Collaborators','PhD Students','Past Members');
	if(isset($_POST['pang_save_order'])){
		check_admin_referer('pang_people_order');
		$json=isset($_POST['pang_order_json'])?json_decode(wp_unslash($_POST['pang_order_json']),true):array();
		if(is_array($json)){
			foreach($json as $ids){
				if(!is_array($ids)) continue;
				foreach($ids as $index=>$id){
					wp_update_post(array('ID'=>(int)$id,'menu_order'=>(int)$index+1));
				}
			}
	...
	foreach($categories as $cat){
		...
		$q=new WP_Query(array('post_type'=>'pang_person','posts_per_page'=>-1,'orderby'=>array('menu_order'=>'ASC','title'=>'ASC'),'tax_query'=>array(array('taxonomy'=>'pang_person_category','field'=>'name','terms'=>$cat))));
		echo '<div class="pang-sortable" data-category="'.esc_attr(sanitize_title($cat)).'">';
		while($q->have_posts()){$q->the_post();echo '<div class="pang-sort-item" data-id="'.intval(get_the_ID()).'">'.esc_html(get_the_title()).'</div>';}
...
.pang-badge--faculty{background:#e4eef9;color:#174f86}.pang-badge--researchers{background:#e5f5eb;color:#247043}
.pang-badge--phd-students{background:#fff4cc;color:#775d00}.pang-badge--collaborators{background:#f0e7f7;color:#68428a}
